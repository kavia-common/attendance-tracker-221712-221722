...
- Required env:
  - JWT_SECRET: secret for signing JWTs (set any non-empty string for local testing). Optional for startup; required when hitting auth endpoints.
...
- A single app instance is created; app.run is only invoked when __name__ == "__main__" to prevent duplicate binding.

Notes:
- All blueprints use flask_smorest.Blueprint and are registered on a flask_smorest.Api instance to avoid AttributeError due to mixed blueprint types when available. If smorest conflicts, direct Flask registration is used.
